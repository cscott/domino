...
Node.prototype = Object.create(EventTarget.prototype, {
  ...
  _insertOrReplace: { value: function _insertOrReplace(parent, before, isReplace) {
    ...
    var kids = parent._firstChildOrChildNodes, first = kids;
    if (Array.isArray(kids)) {
      before_index = (before === null) ? kids.length :
    ...
    if (child.nodeType === DOCUMENT_FRAGMENT_NODE) {
      ...
      if (Array.isArray(kids)) {
        spliceArgs[0] = (before === null) ?
          kids.length : before._index;
        kids.splice.apply(kids, spliceArgs);
        for (i=2; i<len; i++) {
          spliceArgs[i]._index = spliceArgs[0] + (i - 2);
        }
      } else if (parent._firstChildOrChildNodes === before) {
        if (len > 2) {
          parent._firstChildOrChildNodes = spliceArgs[2];
        } else if (isReplace) {
          parent._firstChildOrChildNodes = null;
        }
      }
      // Remove all nodes from the document fragment
      if (Array.isArray(child._firstChildOrChildNodes)) {
        child._firstChildOrChildNodes.length = 0;
      } else {
        child._firstChildOrChildNodes = null;
      }
      ...
      if (isReplace) {
        ...
        if (Array.isArray(kids)) {
          ...
          kids[before_index] = child;
        } else if (first === before) {
          parent._firstChildOrChildNodes = child;
        }
      } else {
        if (n !== null) {
        ...
        if (Array.isArray(kids)) {
          child._index = before_index;
          kids.splice(before_index, 0, child);
        } else if (first === before) {
          parent._firstChildOrChildNodes = child;
        }
      }
