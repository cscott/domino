...
Element.prototype = Object.create(ContainerNode.prototype, {
  ...
  getAttribute: { value: function getAttribute(qname) {
    ...
    if (!attr && this.isHTML) {
      // attrsByQName only has lowercased attributes if this.isHTML, so
      // we can avoid the lowercasing of the attribute name if the
  ...
  hasAttribute: { value: function hasAttribute(qname) {
    if (this.isHTML) {
      // attrsByQName only has lowercased attributes if isHTML, so we can
      // avoid lowercasing the attribute if the attribute is present
      if (this._attrsByQName[qname] !== undefined) { return true; }
      qname = utils.toASCIILowerCase(qname);
    }
  ...
  _getattr: { value: function _getattr(qname) {
    ...
    // null prefix.
    utils.assert(this.isHtml ? !/[A-Z]/.test(qname) : true);
    var attr = this._attrsByQName[qname];
    return attr ? attr.value : null;
  }},
